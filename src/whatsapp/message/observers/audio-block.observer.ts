...
export class AudioBlockObserver implements MessageObserver {
  ...
  async onMessage(sock: WASocket, message: WAMessage) {
    ...
    const response = `> SISTEMA: este número não recebe áudios. A função de bloqueio de áudio está ativada para manter o chat organizado. Envie sua mensagem em texto, por favor.`;
